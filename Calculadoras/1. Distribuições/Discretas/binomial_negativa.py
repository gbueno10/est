...
def calcular_binomial_negativa(
    r = 5,              # número de sucessos desejados (r)
    p = 0.4,            # probabilidade de sucesso (p)
    tipo = 'pmf',       # tipo de cálculo: 'pmf' (P(X=k falhas)), 'cdf', 'sf', 'interval', 'ppf'
    k = 7,              # número de falhas ocorridas (k) antes de atingir r sucessos
    a = 3,              # limite inferior para interval
    b = 12,             # limite superior para interval
    prob = 0.95         # probabilidade acumulada (α) para ppf
):
    """
    Calculadora para Distribuição Binomial Negativa.
    """
...
if __name__ == "__main__":
    calcular_binomial_negativa(
        r = 5,              # número de sucessos desejados (r)
        p = 0.4,            # probabilidade de sucesso (p)
        tipo = 'pmf',       # tipo: 'pmf' (P(X=k falhas)), 'cdf', 'sf', 'interval', 'ppf'
        k = 7,              # número de falhas (k) antes de r sucessos
        a = 3,              # limite inferior para interval
        b = 12,             # limite superior para interval
